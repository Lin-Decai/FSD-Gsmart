...
class CameraNode(Node):
    def __init__(self):
        super().__init__('camera_node_2')
        
        # 初始化CV桥
        self.bridge = CvBridge()
        
        # 创建发布者，发布原始图像和校正后的图像话题
        self.image_publisher = self.create_publisher(Image, 'camera_2/image_raw_2', 30)
        self.corrected_image_publisher = self.create_publisher(Image, 'camera_2/image_corrected_2', 30)
        ...
        # ========== 新增：订阅相机内参话题 ==========
        # 相机2订阅 camera_1/camera_info_2（内参发布节点2发布的话题）
        self.camera_info_subscription = self.create_subscription(
            CameraInfo,
            'camera_1/camera_info_2',  # 订阅相机2的内参话题
        ...
        self.get_logger().info("摄像头节点2已启动，等待接收相机内参...")
        ...
        try:
            ...
            if cap_width != self.image_width or cap_height != self.image_height:
                self.get_logger().warn(
                    f"摄像头2尺寸({cap_width}x{cap_height})与内参尺寸({self.image_width}x{self.image_height})不一致，"
                    f"使用摄像头尺寸进行校正"
                )
