...
class LidarCameraFusionNode(Node):
    ...
    数据流:
        输入订阅:
            ...
            - camera_2/camera_info_2: 相机内参 (CameraInfo)
            - /camera_2/image_raw_2: 相机图像 (Image)
            - detections: YOLO检测结果 (DetectionArray)
            
        输出发布:
            - /image_lidar2: 标注了投影点的图像 (Image)
            - /detected_object_position2: 检测目标的3D位置 (DetectedObjectArray)
            - /detected_object_pointcloud2: 每个目标的点云 (PointCloud2)
    ...
    def __init__(self):
        super().__init__('lidar_to_image_projection2')
        ...
        # 构建配置文件完整路径
        yaml_file_path = os.path.join(package_share_directory, 'config', 'setup_config2.yaml')
        ...
        self.detect_qos = QoSProfile(
            ...
            history=QoSHistoryPolicy.KEEP_LAST,          # 保留所有消息
            depth=10                                     # 队列深度
        )
        ...
        self.camera_info_subscription = self.create_subscription(
            CameraInfo, 'camera_2/camera_info_2', self.camera_info_callback, 10)
        
        # 图像订阅 - 原始相机图像
        self.image_subscription = self.create_subscription(
            Image, '/camera_2/image_raw_2', self.image_callback, 10)
        ...
        # ==================== 发布器 ====================
        # 发布标注了投影点的图像
        self.image_publisher = self.create_publisher(Image, '/image_lidar2', 10)
        
        # 发布检测目标的3D位置信息
        self.detected_object_array_publisher = self.create_publisher(
            DetectedObjectArray, '/detected_object_position2', qos_profile=self.detect_qos)
        
        # 发布每个目标的独立点云
        self.object_pointcloud_publisher = self.create_publisher(
            PointCloud2, '/detected_object_pointcloud2', qos_profile=self.detect_qos)
    ...
    def publish_detected_objects(self, bbox_points, header):
        ...
        detected_objects_msg.header.frame_id = "camera2_link"  # 坐标系标识
