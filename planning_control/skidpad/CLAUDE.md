...
- **Input**: cone positions (`/hesai/cone_positions`, `geometry_msgs/PoseArray`) — from `lidar_cone_detector`
- **Output**: `Float64MultiArray` on `/transform_matrix` (4x4 world-to-vehicle transform, row-major 16 elements)
- **How**: Receives cone centroid positions as PoseArray, converts to PointCloud2 internally, loads a reference PCD map, nearest-neighbor matching against the loaded map, then PCL ICP alignment. Publishes the transform matrix only when the match flag is set.
- **Config**: `src/skidpad_icp/config/skidpad_detector.yaml` — cone topic (`/hesai/cone_positions`), PCD path, distance thresholds, ICP convergence params.
...
### 4. FSDS bridge — `fsds_ros_bridge` (Python, standalone package at `../../fsds_ros_bridge/`, launched only in simulation mode)

- Publishes raw lidar `PointCloud2` on `/lidar_points` (10 Hz, sensor-frame via `getLidarData()`), `CarState` on `/estimation/slam/state` (100 Hz), `Bool` GO signal on `/go_cmd`.
- Subscribes to `ControlCommand` on `/control_command` and forwards it to the FSDS simulator.
- Coordinate transform: world→sensor frame for lidar, ENU→target frame for car state (X=north=straight, Y=west=left).
...
|--------|-----------|--------------|
| Lidar cone detector | Same in both modes (subscribes `/lidar_points`) | Same |
| ICP | Same in both modes (subscribes `/hesai/cone_positions`) | Same |
| Planner | Receives `simulation: true` param | `simulation: false` |
| Control | `mode: simulation` (includes dynamics model) | `mode: real_vehicle` (PID only) |
| Bridge | Launched (`fsds_ros_bridge`) | Not launched |
...
| `skidpad_icp` | C++ | `ament_cmake` | ICP localization (PoseArray input) |
...
/hesai/cone_positions ──[ICP]──> /transform_matrix (Float64MultiArray)
